...
const de_webextApi = {
  download: function(downloadRequest) {
    chrome.runtime.sendMessage(Object.assign(downloadRequest, {
      type: 'download'
    }));
  },
...
const de_util = {
  classStrToArr: function(str) {
    return str?.replace('.', '').split(' .');
  },
  classStrToQryStr: function(str) {
...
const de_settings = {
  setters: {
    ...
    classesForSaveMark: newValue => de_settings.refreshClassesForSaveMark(newValue),
    additionalCss: newValue => de_settings.refreshAdditionalCss(newValue),
    ...
    btnSize: newValue => de_settings.btnSize = parseInt(newValue),
    saveHotkey: newValue => de_settings.saveHotkey = newValue,
    enableSaveHotkey: newValue => de_hotkeys.bindReservedHotkeys() && (de_settings.enableSaveHotkey = newValue),
  },
  ...
  refreshStyleForSaveMark: function(value) {
    ...
    if(value) {
      const newCssString = `.${de_contentscript.saveMarkStyleClass} {${value}}`;
      de_settings.styleForSaveMark = newCssString;
      de_webextApi.switchPageStyle(newCssString, true);
    }
  },

  refreshClassesForSaveMark: function(value) {
    document.querySelectorAll(`.${de_contentscript.saveMarkStyleClass}, ${de_util.classStrToQryStr(de_settings.classesForSaveMark || '.zzz__zzz')}`).forEach(el => {
      !de_settings.classesForSaveMark || el.classList.remove(...de_util.classStrToArr(de_settings.classesForSaveMark));
      !value || el.classList.add(...de_util.classStrToArr(value));
    });
    !value || (de_settings.classesForSaveMark = value);
  },

  refreshAdditionalCss: function(value) {
    if(de_settings.additionalCss) {
      let el = document.querySelector(`#${de_contentscript.idForGlobalCSS}`);
      el.parentNode.removeChild(el);
    }
    if(value) {
      let el = document.createElement('style');
      document.head.appendChild(el);
      el.setAttribute('id', de_contentscript.idForGlobalCSS);
      el.appendChild(document.createTextNode(value));
      de_settings.additionalCss = value;
    }
  },
};
...
const de_button = {
  ...
  overrideAndListen: function(eventType) {
    function listener(event) {
      if(event.target.nodeName !== de_button.name)
        return;
  ...
  eventsHandlers: {
    mouseup: function(eventButton) {
      ...
      if(!btnElem.classList.contains('click') || !that.downloadRequest.src || (de_settings.forbidDuplicateFiles && de_contentscript.downloadsHistory.includes(historyEntry))){ 
        that.unclick();
        return;
      }
      ...
      that.unclick();
      de_contentscript[`${de_contentscript.currentNode.classList.contains(de_contentscript.saveMarkStyleClass) ? 'refresh' : 'add'}SaveMark`]();
    },
  ...
  emulateClick: function(buttonCode = 0) {
    ...
    this.elem.dispatchEvent(new MouseEvent('mouseup', {
      button: buttonCode
    }));
  },
...
const de_contentscript = {
  ...
  downloadsHistory: [],
  saveMarkStyleClass: 'cute-and-saved',
  idForGlobalCSS: 'cute-stylesheet',
  pageInfo: {
    domain: null,
    title: null,
    threadNum: null,
  ...
  rememberDownload: function(url) {
    const historyLen = 50;
    if(!de_settings.forbidDuplicateFiles)
      return;
  ...
  addSaveMark: function() {
    this.currentNode.classList.add(this.saveMarkStyleClass);
    if(de_settings.classesForSaveMark)
      this.currentNode.classList.add(...de_util.classStrToArr(de_settings.classesForSaveMark));
  },

  removeSaveMark: function() {
    this.currentNode.classList.remove(this.saveMarkStyleClass);
    if(de_settings.classesForSaveMark)
      this.currentNode.classList.remove(...de_util.classStrToArr(de_settings.classesForSaveMark));
  },

  refreshSaveMark: function() {
    this.removeSaveMark(), setTimeout(() => this.addSaveMark(), 250);
  },
  ...
  nodeTools: {
    ...
    handleAgainOn: function(eventType, node) {
      node.addEventListener(eventType, () => de_contentscript.nodeHandler(node), {
        once: true
      });
    },
    ...
    filterBySize: function(node, modifier) {
      ...
      if(node.complete && node.naturalWidth >= de_settings.minSize && node.naturalHeight >= de_settings.minSize && (de_settings.checkByRealImageSize || isRenderedSizeBig) // don't check by rendered size if checkByRealImageSize option enabled
      )
  ...
  isTrash: function(node, modifier) {
    ...
    if(tools.checkForBgSrc(node, modifier) || tools.deepSearchHostSpecific(node))
      return false;
    ...
    if(tools.filterBySrc(node[de_contentscript.srcLocation]) || tools.filterByExclusions(node))
      return true;
  ...
  nodeHandler: async function(currentTarget, event = {}) {
    ...
    if(!currentTarget || (!that.isSeparateTab && event.ctrlKey && !event.altKey))
      return;
    ...
    de_button.show(
      Object.assign({
          ...
          right: null,
          bottom: null
        }, // only two position properties would be set at once, other two are null on purpose to reset their default values
...
const de_siteParsers = {
  ...
  getOriginalSrc: async function(node) {
    ...
    const getters = [{
      ...
      get: () => {
        if(node.currentSrc.includes('/images/'))
          return null;
  ...
  getOriginalFilename: function(node) {
    ...
    if(de_contentscript.isSeparateTab && !hostsWithFilenameInSrc.includes(this.host))
      return null;
    ...
    function tryFilenameFromDollchanImageByCenter() {
      if(!de_siteParsers.dollchanImproved)
        return null;
...
const de_events = {
  listeners: {
    mouseover: function(event) {
      if(event.target.tagName === de_button.name || (event.relatedTarget && event.relatedTarget.tagName === de_button.name)) {
        return;
      }
      try {
    ...
    keydown: function(e) {
      if(de_hotkeys.isHotkeyPossible(e) && de_hotkeys.isHotkeyExists(de_hotkeys.buildHotkeyId(e)))
        e.preventDefault();
    },
    keyup: function(e) {
      let id = de_hotkeys.buildHotkeyId(e);
      if(!de_hotkeys.isHotkeyPossible(e) || !de_hotkeys.isHotkeyExists(id))
        return;

      if(de_contentscript.isSeparateTab && !de_button.isVisible())
        de_contentscript.nodeHandler(document.body.childNodes[0]);

      de_hotkeys.selectedKeyboardRule = de_hotkeys.keyboardHotkeys[id];
  ...
  listen: function(eventType) {
    window.addEventListener(eventType, de_events.listeners[eventType], {
      capture: true
    });
  },

  unlisten: function(eventType) {
    window.removeEventListener(eventType, de_events.listeners[eventType], {
      capture: true
    });
  },
};
...
const de_hotkeys = {
  ...
  bindReservedHotkeys: function() {
    this.assignHotkeyRule(Object.assign({
      mouseButton: 0,
      id: '333'
    }, de_hotkeys.fallbackRule)); // Space, save to default location
  },

  buildHotkeyId: function(e) {
    let saveHotkey = true;
    for(let val of ['ctrlKey', 'altKey', 'metaKey', 'shiftKey', 'key']) {
      let [a, b] = typeof e[val] == 'string' ? [de_settings.saveHotkey[val].toUpperCase(), e[val].toUpperCase()] : [de_settings.saveHotkey[val], e[val]];
      if(a !== b) {
        saveHotkey = false;
        break;
      }
    }
    return saveHotkey ? '333' : `${e.ctrlKey ? 1 : 0}${e.altKey ? 1 : 0}${e.shiftKey ? 1 : 0}${e.keyCode}`;
  },

  isHotkeyPossible: function(e) {
    return (
      (de_contentscript.isSeparateTab && this.isNoScroll()) ||
      (
        de_button.isVisible() &&
        !['INPUT', 'TEXTAREA'].includes(e.target.tagName) &&
        !e.target.classList.contains('submit_post_field') // vk inputs
      )
    );
  },
  ...
  assignHotkeyRule: function(rule) {
    ...
    if(!that.isRuleForCurrentDomain(rule))
      return;
